...
# Function to process a DataFrame with SMILES strings
def process_data(df):
    all_descriptors = []
    for smiles in df['SMILES']:
        descriptors = smiles_to_descriptors([smiles])
        if descriptors is not None:
            all_descriptors.append(descriptors.iloc[0])
        else:
            all_descriptors.append(pd.Series([0]*len(selected_features), index=selected_features))
    
    processed_df = pd.DataFrame(all_descriptors)
    return processed_df
...
def predict():
    ...
    if not smiles and not file:
        return render_template('index.html', error='Please provide a SMILES string or upload a CSV file.')

    if smiles:
        try:
            processed_data = process_single_smiles(smiles)
            prediction = lgbm.predict(processed_data)
            probability = lgbm.predict_proba(processed_data)[0][1]
            return render_template('index.html', prediction=True, result=prediction[0], probability=probability)
        except Exception as e:
            return render_template('index.html', error=f'Error processing SMILES string: {e}')
    ...
    if file:
        ...
        try:
            ...
            if 'SMILES' not in df.columns:
                return render_template('index.html', error='CSV file must contain a column named "SMILES".')
            # Process the data
            processed_data = process_data(df)
            if processed_data is None or processed_data.empty:
                return render_template('index.html', error='Invalid SMILES string or unable to compute descriptors.')
            # Predict using the model
            predictions = lgbm.predict(processed_data)
            probabilities = lgbm.predict_proba(processed_data)[:, 1]  # Get the probability of class 1
            # Add predictions and probabilities to the DataFrame
            df['predictions'] = predictions
            df['probabilities'] = probabilities
            # Save the results to a new CSV file
            result_csv_path = '/home/liushui1986/result.csv'
            df.to_csv(result_csv_path, index=False)
            return send_file(result_csv_path, as_attachment=True)
        except Exception as e:
            return render_template('index.html', error=f'Error processing CSV file: {e}')
